...
const init = {
    ...
    toolbar:
        "formats undo redo fontsizeselect fontselect ltr rtl searchreplace media imageUpload | outdent indent aligncenter alignleft alignright alignjustify lineheight underline quicklink h2 h3 blockquote numlist bullist table removeformat forecolor backcolor bold italic strikethrough hr link preview fullscreen help ", content_style: "p {margin: 5px 0; font-size: 14px}",
    ...
    setup: (editor) => {
        editor.ui.registry.addButton('imageUpload', {
            ...
            onAction() {
                ...
                handleChoose.value = (e) => {
                    imgUrls.value = e.value.map(item => item.url)
                    console.log(imgUrls.value);
                    imgUrls.value.forEach(url => {
